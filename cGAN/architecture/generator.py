...
class Generator(nn.Module):
    def __init__(self):
        ...
        if sn_flag:
            ...
            print("Generator spectral norm disabled")
            self.fc = (nn.Linear(self.z_dim_chunk, 4 * 4 * 4 * nf))
            self.embedding = (nn.Linear(1000, self.embedding_size,bias=True))
...
class CondBN(nn.Module):
    def __init__(self, nc, cond_dim):
        ...
        if sn_flag:
            ...
            self.mlp_gamma = (nn.Linear(cond_dim, nc))
            self.mlp_beta = (nn.Linear(cond_dim, nc))
