...
from .activation import SReLU
from .oneshot import OneshotNormalizer2D
def stats(x):
    with torch.no_grad():
        if x.dim() == 4:
            #print(x.shape)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, affine=True,bn=True,bias=False,res=True,scale=1.0,momentum=0.1):
        ...
        if self.bn:
            ...
            self.conv1.add_module('oneshot',OneshotNormalizer2D())
            self.conv2.add_module('oneshot',OneshotNormalizer2D())
            print("adding bn gain")
            gain = torch.ones((1,out_channels * BasicBlock.expansion,1,1))
            self.gain_view = (1,out_channels * BasicBlock.expansion,1,1)
            gain = torch.squeeze(gain)
            self.bn_gain = nn.Parameter(gain,requires_grad=True)
            #print(self.bn_gain[0,0,0,0])
    ...
    def forward(self, x):
        self.steps += 1
        if self.res:
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(x)
                    #print(self.bn_gain[0,0:3,0,0])
                    print("=" *20)
                    print("block input mean: ",mean, " std: ",std)

            output = SReLU(inplace=True)(x)
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("relu1 mean: ",mean, " std: ",std)
            skip = self.shortcut(output)
            #print(skip.shape)
            ...
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("block conv1 mean: ",mean, " std: ",std)

            output = SReLU(inplace=True)(output) * self.scale

            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("block relu1 mean: ",mean, " std: ",std)
            output = self.conv2(output)

            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("block conv2 mean: ",mean, " std: ",std)
            
            if not self.bn and self.affine:
                # with torch.no_grad():
                #     normalizer = torch.sqrt(torch.tensor(1) + self.bn_gain.pow(2).view(self.gain_view))
                # output = (skip + output * self.bn_gain.view(self.gain_view))/ normalizer 
                if True:
                    with torch.no_grad():
                        normalizer = torch.sqrt(torch.tensor(1) + self.bn_gain.pow(2).view(self.gain_view))
                    output = (skip + output * self.bn_gain.view(self.gain_view))/ normalizer 
                    ...
                    with torch.no_grad():
                        normalizer = torch.sqrt(1 - torch.clamp_max(self.bn_gain ** 2,1.0)).view(self.gain_view)
                    output = skip/ normalizer + torch.clamp(self.bn_gain,-1,1).view(self.gain_view) * output
            else:
                with torch.no_grad():
                    normalizer = torch.sqrt(torch.tensor(2))
                output = (output + 1 * skip)/ normalizer ** 2
            ...
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    ...
                    #print(normalizer)
                    #print(self.bn_gain[0,0,0,0])
                    print("block skip + resitual mean: ",mean, " std: ",std)
                    print("=" *20)
...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, affine=True,bn=True,bias=False,res=True,scale=1.0,momentum=0.1):
        ...
        if self.bn:
            ...
            print("adding bn gain")
            gain = torch.ones((1,out_channels * BottleNeck.expansion,1,1))
            self.gain_view = (1,out_channels * BottleNeck.expansion,1,1)
            gain = torch.squeeze(gain)
            self.bn_gain = nn.Parameter(gain,requires_grad=True)
        self.shortcut = nn.Sequential()
    ...
    def forward(self, x):
        self.steps += 1
        if self.res:
            output = SReLU(inplace=True)(x) * self.scale
            skip = self.shortcut(output)
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(skip)
                    print("skip mean: ",mean, " std: ",std)
            output = self.conv1(output)
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
            ...
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("conv1 relu mean: ",mean, " std: ",std)
            output = self.conv2(output)
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("conv2 mean: ",mean, " std: ",std)
            output = SReLU(inplace=True)(output) * self.scale
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("conv2 relu mean: ",mean, " std: ",std)
            output = self.conv3(output)
            if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("conv3 mean: ",mean, " std: ",std)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.conv1(x) * 2 

        if self.pf and (self.steps % self.N == 0):
                with torch.no_grad():
                    mean,std = stats(output)
                    print("stem mean: ",mean, " std: ",std)
